 ...
 * ~/.claude/channels/discord/access.json — managed by the /artifice-discord:access skill.
 *
...
if (!TOKEN) {
  process.stderr.write(
    `artifice-discord: DISCORD_BOT_TOKEN required\n` +
    `  set in ${ENV_FILE}\n` +
...
process.on('unhandledRejection', err => {
  process.stderr.write(`artifice-discord: unhandled rejection: ${err}\n`)
})
process.on('uncaughtException', err => {
  process.stderr.write(`artifice-discord: uncaught exception: ${err}\n`)
})
...
function readAccessFile(): Access {
  try {
    ...
    process.stderr.write(`artifice-discord: access.json is corrupt, moved aside. Starting fresh.\n`)
...
const BOOT_ACCESS: Access | null = STATIC
  ? (() => {
      ...
      if (a.dmPolicy === 'pairing') {
        process.stderr.write(
          'artifice-discord: static mode — dmPolicy "pairing" downgraded to "allowlist"\n',
        )
...
// The /artifice-discord:access skill drops a file at approved/<senderId> when it pairs
  ...
  for (const senderId of files) {
    ...
    void (async () => {
      try {
        ...
        process.stderr.write(`artifice-discord: failed to send approval confirm: ${err}\n`)
...
async function fetchAllowedChannel(id: string) {
  ...
  throw new Error(`channel ${id} is not allowlisted — add via /artifice-discord:access`)
}
...
const mcp = new Server(
  { name: 'artifice-discord', version: '1.0.0' },
  {
    capabilities: {
      tools: {},
      experimental: {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'voice') {
      ...
      try {
        ...
        // Ack first so the 3-second Discord deadline doesn't constrain the polling loop.
        await interaction.reply({ content: `✓ Switching to \`${modelId}\`...` }).catch(e => {
          process.stderr.write(`artifice-discord: /model reply failed: ${e}\n`)
        })
        // "Switch model?" prompt timing varies — poll up to 6s and auto-confirm.
        for (let i = 0; i < 24; i++) {
...
async function handleInbound(msg: Message): Promise<void> {
  ...
  if (result.action === 'pair') {
    ...
    try {
      await msg.reply(
        `${lead} — run in Claude Code:\n\n/artifice-discord:access pair ${result.code}`,
      )
    } catch (err) {
      process.stderr.write(`artifice-discord: failed to send pairing code: ${err}\n`)
    }
    return
  }
  ...
  mcp.notification({
    ...
    process.stderr.write(`artifice-discord: failed to deliver inbound to Claude: ${err}\n`)
  })
}
...
client.login(TOKEN).catch(err => {
  process.stderr.write(`artifice-discord: login failed: ${err}\n`)
  process.exit(1)
})
